...
export default function MyVerticallyCenteredModal(props: Props) {
	const [page, setPage] = useState(props.startValue);
	const { data: session } = useSession();

	const handleSubmit = async (event: BaseSyntheticEvent) => {
		event.preventDefault();
		const name = (document.getElementById('server_name') as HTMLInputElement).value;
		const guildIcon = (document.getElementById('server_icon') as HTMLInputElement)?.value;

		console.log(name, guildIcon);
		// Send request to API
		const res = await fetch(`${process.env.NEXT_PUBLIC_APIURL}/api/guilds`, {
			method: 'post',
			headers: {
	...
	function pagenator(index: number) {
		switch (index) {
			case 0:
				// Main page
				return (
					<div>
						<img alt="" src="/img/icons/createServer.svg" />
						<button type="button" name="button" onClick={()=> setPage(2)}>Create your own</button>
						<h6 style={{ color:'black' }}>Already have an invite?</h6>
						<button type="button" name="button" className="btn btn-secondary" style={{ width:'100%' }} onClick={() => setPage(1)}>Join a server</button>
					</div>
				);
			case 1:
			// Join an existing page
				return (
					<div className="modal-split">
						<div className="modal-header">
							<h5 className="modal-title" id="exampleModalLongTitle" style={{ color:'black' }}>Join a server</h5>
							<button type="button" className="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
						<p className="text-center">Enter an invite below to join an existing server</p>
						<div className="modal-body">
							<form>
								<div className="form-group">
									<label>INVITE LINK</label>
									<input type="text" className="form-control" placeholder="8g6zUQu" />
									<small id="emailHelp" className="form-text text-muted">Invites should look like this: <br />8g6zUQu</small>
								</div>
								<a type="button" onClick={() => setPage(0)}>Back</a>
								<button type="submit" className="btn btn-primary" style={{ float:'right' }}>Join server</button>
							</form>
						</div>
					</div>
				);
						...
						<div className="modal-body">
							<form onSubmit={handleSubmit} method="post">
								<div style={{ margin: 'auto', width: '40%' }}>
									...
									<input className="file-input" type="file" id="server_icon" accept=".jpg,.jpeg,.png,.gif" aria-label="Upload a Server Icon"></input>
								</div>
								<div className="form-group">
									<label>Server name</label>
									<input type="text" className="form-control" id="server_name" />
									<small id="emailHelp" className="form-text text-muted">By creating a server, you agree to the <a href="/guidlines">Community Guidlines</a>.</small>
								</div>
								<a type="button" onClick={() => setPage(0)}>Back</a>
